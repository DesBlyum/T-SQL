...
ТабСтруктура AS (
			...
			FROM 
				#ТабАлкоголь (nolock) ТабАлкоголь
...
Приходы AS (
			SELECT  
				   ...
				   Количество
			FROM 
				Документы_Приходы (nolock)
			INNER JOIN #ТабАлкоголь as Алкоголь ON Алкоголь.CODE = ТоварКод
			WHERE 
				Дата between '20220701' and '20221231' 
),	
...
ПеремещенияРозница AS (
			SELECT 
				...
				Дата
			FROM 
				Документы_ПеремещениеРозница (nolock)
			INNER JOIN #ТабАлкоголь as Алкоголь ON Алкоголь.CODE = ТоварКод
			WHERE 
				Дата between '20220701' and '20221231'
),	
...
Перемещения AS (
			...
			FROM 
				ПеремещенияРозница
			WHERE 
				СкладОтправительКод not in (1,2,5) and ДокументОснование = '   0     0   '
),
...
СписанияОприходования AS (
			SELECT 
				...
				Количество
			FROM 
				Документы_КорректировкаСклада (nolock)
			INNER JOIN #ТабАлкоголь as Алкоголь ON Алкоголь.CODE = ТоварКод
			WHERE 
				Дата between '20220701' and '20221231'  
),	
...
FROM 
	ТабОстаткиПоСкладам ОстаткиПоСкладам

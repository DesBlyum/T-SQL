...
BEGIN
	...
	ПродажиПоЧекам as (
			SELECT
				...
				COUNT(*) POZ
			FROM 
				CASH_CHECK_DT (nolock)
			WHERE 
				[DATE] between @НачалоПервогоПериода and @КонецТекущегоПериода and PROD_CODE in (SELECT code FROM СписокНоменклатуры)
			GROUP BY 
				[DATE],
				SKLAD_CODE,
				NDOC
	...
	ДанныеЧекаШ as (
			...
			WHERE 
				DATEDOC between @НачалоПервогоПериода and @КонецТекущегоПериода
	),
	...
	WHERE
		ДС1.ИдентификаторПериода = @ИдентифкаторПервогоПериода 	and
		Справочник_Склады.Действующий = 'Да' 			and 
		Справочник_Склады.ТипСклада = 'Розница'
	ORDER BY
